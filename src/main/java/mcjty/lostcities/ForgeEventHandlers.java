    ...
    public void onCreateSpawnPoint(WorldEvent.CreateSpawnPosition event) {
        ...
        if (!world.isRemote) {
            ...
            if (!profile.SPAWN_BIOME.isEmpty()) {
                ...
                if (spawnBiome == null) {
                    MrCrayfishMod.setup.getLogger().error("Cannot find biome '" + profile.SPAWN_BIOME + "' for the player to spawn in !");
                } else {
                ...
                if (city == null) {
                    MrCrayfishMod.setup.getLogger().error("Cannot find city '" + profile.SPAWN_CITY + "' for the player to spawn in !");
                } else {
                ...
                if ("<in>".equals(profile.SPAWN_SPHERE)) {
                    ...
                    if (sphere == null) {
                        MrCrayfishMod.setup.getLogger().error("Cannot find sphere '" + profile.SPAWN_SPHERE + "' for the player to spawn in !");
                    } else {
...
//        int bottom = world.getWorldType().getMinimumSpawnHeight(world);
        while (true) {
            ...
            radius += 100;
            if (attempts > 10000) {
                MrCrayfishMod.setup.getLogger().error("Can't find a valid spawn position!");
    ...
    public void onPlayerlogin(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        if(event.player.dimension == 10086){
            return;
        }
        BlockPos bedLocation = event.player.getPosition();
        WorldServer destWorld = event.player.getEntityWorld().getMinecraftServer().getWorld(LostCityConfiguration.DIMENSION_ID);
        BlockPos location = findLocation(bedLocation, destWorld);
        CustomTeleporter.teleportToDimension(event.player, LostCityConfiguration.DIMENSION_ID, location);
    }
