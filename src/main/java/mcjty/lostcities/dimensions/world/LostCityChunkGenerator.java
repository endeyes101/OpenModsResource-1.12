...
public class LostCityChunkGenerator implements IChunkGenerator, ILostChunkGenerator {
    ...
    public Chunk generateChunk(int chunkX, int chunkZ) {
        ...
        try {
            ...
            MrCrayfishMod.setup.getLogger().error("An exception occured while generating chunk: " + chunkX + "," + chunkZ, e);
            BuildingInfo info = BuildingInfo.getBuildingInfo(chunkX, chunkZ, this);
            MrCrayfishMod.setup.getLogger().error("    Chunk profile: " + info.profile.getName());
            MrCrayfishMod.setup.getLogger().error("    Is City: " + info.isCity());
            MrCrayfishMod.setup.getLogger().error("    Building type: " + info.getBuildingType());
            MrCrayfishMod.setup.getLogger().error("    City level: " + info.getCityLevel());
            MrCrayfishMod.setup.getLogger().error("    City ground level: " + info.getCityGroundLevel());
            MrCrayfishMod.setup.getLogger().error("    Num floors: " + info.getNumFloors());
            MrCrayfishMod.setup.getLogger().error("    Num cellars: " + info.getNumCellars());
            throw (e);
        }
            ...
            if (world.getBlockState(pos).getBlock() == Blocks.MOB_SPAWNER) {
                ...
                if (tileentity instanceof TileEntityMobSpawner) {
                    ...
                    if (LostCityConfiguration.DEBUG) {
                        MrCrayfishMod.setup.getLogger().debug("generateLootSpawners: mob=" + randomValue + " pos=" + pos.toString());
                    }
                } else if(tileentity != null) {
                    MrCrayfishMod.setup.getLogger().error("The mob spawner at (" + pos.getX() + ", " + pos.getY() + ", " + pos.getZ() + ") has a TileEntity of incorrect type " + tileentity.getClass().getName() + "!");
                } else {
                    MrCrayfishMod.setup.getLogger().error("The mob spawner at (" + pos.getX() + ", " + pos.getY() + ", " + pos.getZ() + ") is missing its TileEntity!");
                }
            }
        }
        ...
        for (Pair<BlockPos, BuildingInfo.ConditionTodo> pair : info.getLootTodo()) {
            ...
            if (te instanceof TileEntityLockableLoot) {
                ...
                if (block.hasTileEntity(state)) {
                    MrCrayfishMod.setup.getLogger().error("The block " + block.getRegistryName() + " (" + block.getClass().getName() + ") at (" + pos.getX() + ", " + pos.getY() + ", " + pos.getZ() + ") is missing its TileEntity!");
                }
            }
        }
        info.clearLootTodo();
    ...
    private void createLoot(BuildingInfo info, Random random, World world, BlockPos pos, BuildingInfo.ConditionTodo todo) {
        ...
        if (tileentity instanceof TileEntityLockableLoot) {
            if (todo != null) {
                ...
                if (LostCityConfiguration.DEBUG) {
                    MrCrayfishMod.setup.getLogger().debug("createLootChest: loot=" + randomValue + " pos=" + pos.toString());
                }
            }
        }
    }
