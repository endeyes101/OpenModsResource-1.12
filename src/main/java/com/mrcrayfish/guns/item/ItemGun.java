...
public class ItemGun extends ItemColored
{
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn)
    {
        ...
        if(worldIn.isRemote)
        {
            if(!MrCrayfishMod.proxy.canShoot())
            {
            ...
            {
                MrCrayfishMod.proxy.playClientSound(SoundEvents.BLOCK_LEVER_CLICK);
            }
        }
        return new ActionResult<>(EnumActionResult.FAIL, heldItem);
    }
    ...
    public static void fire(World worldIn, EntityPlayer playerIn, ItemStack heldItem)
    {
        ...
        {
            MrCrayfishMod.logger.info(playerIn.getName() + "(" + playerIn.getUniqueID() + ") tried to fire before cooldown finished or server is lagging?");
            return;
        }
